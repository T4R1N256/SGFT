...
MAPA = RAIZ_SKILL / "ownership" / "ownership.json"
...
def aprobadores(regla, mapa):
    ...
    claves = (
        [regla["dueno"]] + regla.get("colaboradores", []) + regla.get("revisor", [])
    )
...
def imprimir_ruta(ruta, mapa):
    ...
    if regla is None or regla["patron"] == "**":
        print(
            "  ⚠ Fuera de la estructura aprobada. No crear el archivo: pedir ubicación y dueño al integrador (Tarín)."
        )
        return
    ...
    if mapa.get("aprobador_general"):
        print(
            f"  Aprobador general: {nombre(mapa['aprobador_general'], mapa)} — su aprobación basta (DEC-18)"
        )
    print(f"  Paquetes WBS     : {', '.join(regla.get('wbs', [])) or '—'}")
    if regla.get("en_evaluacion"):
        print(
            "  ⏸ EN EVALUACIÓN : paquete fuera de la línea base; NO crear el archivo hasta que el equipo lo apruebe"
        )
    if regla.get("pareja_obligatoria"):
        print(
            "  Pareja           : OBLIGATORIA (ADR-02 §6) — ningún archivo con un solo autor"
        )
    for f in regla.get("funciones_con_otro_autor", []):
        print(
            f"  Función ajena    : {f['funcion']} → autor {f['autor']} (WBS {f['wbs']}). {f['motivo']}"
        )
...
def imprimir_persona(clave, mapa):
    if clave not in mapa["integrantes"]:
        sys.exit(
            f"Persona desconocida: {clave}. Opciones: {', '.join(mapa['integrantes'])}"
        )
    ...
    secciones = {
        "Dueño de": [],
        "Colabora / pareja en": [],
        "Revisor obligatorio de": [],
        "Escribe funciones en archivos ajenos": [],
    }
    for r in mapa["reglas"]:
        ...
        for f in r.get("funciones_con_otro_autor", []):
            if clave in f["autor"]:
                secciones["Escribe funciones en archivos ajenos"].append(
                    f"{r['patron']} → {f['funcion']} [WBS {f['wbs']}]"
                )
...
def imprimir_wbs(codigo, mapa):
    ...
    for r in mapa["reglas"]:
        ...
        funciones = [
            f for f in r.get("funciones_con_otro_autor", []) if codigo in f["wbs"]
        ]
        if directo or funciones:
            hallado = True
            print(
                f"  • {r['patron']}: dueño {nombre(r['dueno'], mapa)}"
                + (
                    f"; colaboran {lista(r.get('colaboradores', []), mapa)}"
                    if r.get("colaboradores")
                    else ""
                )
                + (
                    f"; revisa {lista(r.get('revisor', []), mapa)}"
                    if r.get("revisor")
                    else ""
                )
            )
    ...
    if not hallado:
        print(
            "  Sin rutas registradas. Revisar references/matriz-wbs-responsables.md (puede ser un entregable fuera del repositorio)."
        )
...
def generar_codeowners(mapa):
    ...
    for _, r in ordenadas:
        cuentas = " ".join(
            mapa["integrantes"][c]["github"] for c in aprobadores(r, mapa)
        )
...
def revisar(rutas, mapa):
    ...
    for ruta in rutas:
        ...
        if r is None or r["patron"] == "**":
            dueno, aprueban, estado = (
                "—",
                "(el integrador decide)",
    ...
    if avisos:
        print(
            f"⚠ {len(avisos)} ruta(s) con aviso: confirma la coautoría (Co-authored-by) o que el autor de la función ajena revisó."
        )
    if bloqueos:
        print(
            f"✗ {len(bloqueos)} ruta(s) bloquean la fusión: ubícalas en assets/ownership.json o espera la aprobación del paquete."
        )
        return 1
...
def verificar(mapa):
    ...
    for r in mapa["reglas"]:
        ...
        if len(personas) < 2:
            errores.append(
                f"{r['patron']}: solo una persona; con revisión obligatoria bloquearía sus propios PR"
            )
    ...
    if errores:
        ...
        return 1
    print(
        f"✓ Mapa consistente: {len(mapa['reglas'])} reglas, {len(CASOS_ESPERADOS)} casos verificados, CODEOWNERS equivalente."
    )
    return 0
...
def main():
    ...
    p.add_argument(
        "--salida",
        help="con --codeowners: escribe el archivo en esta ruta (UTF-8) en vez de imprimirlo",
    )
    p.add_argument("--verificar", action="store_true")
    p.add_argument(
        "--revisar",
        action="store_true",
        help="revisa las rutas de un PR (como argumentos o una por línea en la entrada estándar)",
    )
    ...
    if a.codeowners:
        ...
        if a.salida:
            ...
            pendientes = sorted(
                c
                for c, p in mapa["integrantes"].items()
                if p["github"].startswith("@TODO_")
            )
            print(f"✓ CODEOWNERS escrito en {destino}")
            if pendientes:
                print(
                    f"  ⚠ Falta el usuario de GitHub de: {', '.join(pendientes)} — defínelo en assets/ownership.json (campo github) y regenera."
                )
        else:
    ...
    if a.persona:
        clave = (
            unicodedata.normalize("NFKD", a.persona)
            .encode("ascii", "ignore")
            .decode()
            .lower()
            .split()[0]
